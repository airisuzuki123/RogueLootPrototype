...
@export var arena_position: Vector2 = Vector2.ZERO
@export var arena_size: Vector2 = Vector2(1552.0, 852.0)
...
func get_arena_rect() -> Rect2:
	if center_in_viewport:
		...
		var position := Vector2(
			maxf(0.0, (viewport_size.x - arena_size.x) * 0.5),
			maxf(0.0, (viewport_size.y - arena_size.y) * 0.5)
		) + arena_position
		return Rect2(position, arena_size)
