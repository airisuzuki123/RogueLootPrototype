...
func _fire_close_slash() -> void:
	...
	for enemy_node in get_tree().get_nodes_in_group("enemies"):
		...
		if enemy.has_method("take_damage"):
			...
			hit_count += 1
			if hit_count <= 12:
				CombatFeedback.show_burst(world, impact_position, Color(0.76, 1.0, 0.48, 0.82), 0.72 + float(close_slash_stacks) * 0.04)
	_show_close_slash_effect(radius)
	if hit_count > 0:
		if guard_blade_stacks > 0:
			apply_graze_shield(guard_blade_stacks * 2, 2.5)
	CombatFeedback.show_burst(world, global_position, Color(0.62, 1.0, 0.58, 0.74), 1.25 + float(close_slash_stacks) * 0.18)

func _show_close_slash_effect(radius: float) -> void:
	var world := get_tree().current_scene
	var sweep_direction := -1.0 if attack_sequence % 2 == 0 else 1.0
	var rotation_offset := float(attack_sequence % 12) * 0.48
	var stack_width := float(close_slash_stacks) * 0.32
	CombatFeedback.show_ring(world, global_position, radius, Color(0.72, 1.0, 0.55, 0.58), 3.4 + stack_width, 0.34)
	CombatFeedback.show_arc(world, global_position, radius * 0.88, rotation_offset, sweep_direction * PI * 1.45, Color(0.56, 1.0, 0.38, 0.32), 12.0 + stack_width, 0.34, sweep_direction * 0.58, 0.70)
	CombatFeedback.show_arc(world, global_position, radius * 0.92, rotation_offset + sweep_direction * 0.16, sweep_direction * PI * 1.24, Color(0.94, 1.0, 0.68, 0.92), 5.8 + stack_width, 0.25, sweep_direction * 0.68, 0.80)
	CombatFeedback.show_arc(world, global_position, radius * 0.62, rotation_offset + PI, -sweep_direction * PI * 0.86, Color(0.80, 1.0, 0.54, 0.68), 3.6 + stack_width * 0.7, 0.22, -sweep_direction * 0.44, 0.76)
...
func _fire_pulse_field() -> void:
	...
	for enemy_node in get_tree().get_nodes_in_group("enemies"):
		...
		if enemy.has_method("take_damage"):
			...
			hit_count += 1
			if hit_count <= 12:
				CombatFeedback.show_burst(world, impact_position, Color(0.32, 0.9, 1.0, 0.78), 0.68 + float(pulse_field_stacks) * 0.04)
	_show_pulse_field_effect(radius)
	if hit_count > 0:
		if guard_blade_stacks > 0:
			apply_graze_shield(guard_blade_stacks * 2, 2.5)
	CombatFeedback.show_burst(world, global_position, Color(0.35, 0.82, 1.0, 0.52), 1.45 + float(pulse_field_stacks) * 0.16)

func _show_pulse_field_effect(radius: float) -> void:
	var world := get_tree().current_scene
	var outer_width := 4.0 + float(pulse_field_stacks) * 0.35
	CombatFeedback.show_ring(world, global_position, radius, Color(0.34, 0.86, 1.0, 0.42), outer_width * 0.72, 0.38)
	CombatFeedback.show_expanding_ring(world, global_position, radius, Color(0.30, 0.88, 1.0, 0.82), outer_width + 2.0, 0.34, 0.18)
	CombatFeedback.show_expanding_ring(world, global_position, radius * 0.68, Color(0.70, 0.98, 1.0, 0.62), 3.0 + float(pulse_field_stacks) * 0.24, 0.27, 0.16)
	var ray_count := mini(9, 5 + pulse_field_stacks)
	...
	for index in range(ray_count):
		...
		var start := global_position + direction * radius * 0.22
		var end := global_position + direction * radius * 0.92
		CombatFeedback.show_line(world, start, end, Color(0.46, 0.94, 1.0, 0.54), 3.0 + float(pulse_field_stacks) * 0.18, 0.25)
