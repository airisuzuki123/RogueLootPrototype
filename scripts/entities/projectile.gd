...
func _configure_visuals() -> void:
	...
	if power_tags.has("blast"):
		...
		trail_color = Color(0.35, 1.0, 0.68, 0.38)
	if power_tags.has("critical"):
		projectile_color = Color(1.0, 0.94, 0.45, 1.0)
		glow_color = Color(1.0, 0.82, 0.2, 0.24)
		trail_color = Color(1.0, 0.86, 0.28, 0.38)
		visual_scale += 0.08
	if power_tags.has("multi"):
		visual_scale += 0.05
		trail_color.a = minf(0.52, trail_color.a + 0.08)
	if power_tags.has("charged"):
		glow_color.a = minf(0.36, glow_color.a + 0.08)
		core_color = Color(1.0, 0.98, 0.72, 0.52)
	if visual != null:
		visual.color = projectile_color
		visual.scale = Vector2.ONE * visual_scale
	if glow != null:
		glow.color = glow_color
		glow_base_scale = Vector2.ONE * (visual_scale + 0.1)
		glow.scale = glow_base_scale
	if trail != null:
		trail.width = 4.0 * visual_scale
	...
	if trail_core != null:
		trail_core.width = 1.4 * visual_scale
...
func _on_body_entered(body: Node) -> void:
	...
	if power_tags.has("blast"):
		burst_size = 1.4
	elif power_tags.has("critical"):
		burst_size = 1.15
	if power_tags.has("pierce"):
		burst_size += 0.12
...
func _get_hit_burst_color() -> Color:
	if power_tags.has("blast"):
		return Color(1.0, 0.55, 0.18, 0.95)
	if power_tags.has("critical"):
		return Color(1.0, 0.85, 0.22, 0.95)
	if power_tags.has("pierce"):
		return Color(0.38, 1.0, 0.68, 0.85)
	return Color(0.8, 0.95, 1.0, 0.8)
